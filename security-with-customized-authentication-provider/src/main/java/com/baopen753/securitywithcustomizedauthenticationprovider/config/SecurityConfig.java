...
public class SecurityConfig {
    ...
    public SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(csrfConfig -> csrfConfig.disable()).authorizeHttpRequests(request -> request.requestMatchers("/myAccount", "/myCard", "/myLoans", "/myBalance").authenticated()  // these are protected endpoints
                .requestMatchers("/hello", "/notices", "/register", "/contact").permitAll()).formLogin(Customizer.withDefaults()).httpBasic(Customizer.withDefaults());
        return http.build();
    }
