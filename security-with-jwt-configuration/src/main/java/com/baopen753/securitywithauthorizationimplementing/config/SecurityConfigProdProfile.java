    ...
    public class SecurityConfigProdProfile {
        ...
        public SecurityFilterChain securityFilterChain(HttpSecurity http, RequestContextFilter requestContextFilter) throws Exception {
            ...
            http.csrf(csrfConfig -> csrfConfig.csrfTokenRequestHandler(new CsrfTokenRequestAttributeHandler()).ignoringRequestMatchers("/contact", "/register")    // public IP with POST, PUT, DELETE request should not require CSRF Token
                    ...
                    .addFilterBefore(new JwtValidatorFilter(), BasicAuthenticationFilter.class);       // for upcomming request, validate jwt before process authentication
            ...
            http.authorizeHttpRequests(requests -> requests.requestMatchers("/myAccount").hasAuthority("VIEW_ACCOUNT").requestMatchers("/myCard").hasAuthority("VIEW_CARDS").requestMatchers("myBalance").hasAuthority("VIEW_BALANCE").requestMatchers("myLoans").hasAuthority("VIEW_LOANS").requestMatchers("user").authenticated().requestMatchers("/hello", "/contact", "/register", "/invalidSession", "/notices").permitAll());
